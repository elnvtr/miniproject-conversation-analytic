...
# ---------- TODO 1: LLM (key dari Secrets, bukan hardcode) ----------
try:
...
# ---------- Database: PostgreSQL lokal di Colab ----------
@st.cache_resource
def get_engine():
...
# ---------- TODO 3: generate_sql (+ backoff untuk 429/503) ----------
    ...
    for i in range(maks_coba):
        try:
            ...
            if any(k in str(e).lower() for k in ["429", "503", "resource"]) and i < maks_coba - 1:
                time.sleep(5 * (i + 1)); continue   # tunggu makin lama tiap gagal
            raise
...
# ---------- TODO 5: chart (return fig kalau cocok, else None) ----------
def buat_chart(df):
...
# ---------- TODO 6: pipeline (generate -> validate(+retry) -> run) ----------
def jawab(question):
    sql = generate_sql(question)
    if not validate_sql(sql):                       # retry 1x bila tidak lolos
        sql = generate_sql(question)
...
# ---------- UI Chat (st.session_state + chat_input + chat_message) ----------
if "messages" not in st.session_state:
    st.session_state.messages = []

with st.sidebar:                                    # contoh pertanyaan
...
for m in st.session_state.messages:                 # render riwayat
...
q = st.chat_input("Tanya tentang aset & gangguan…")  # input baru
if q:
